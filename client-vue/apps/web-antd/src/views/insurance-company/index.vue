...
const [Grid, gridApi] = useVbenVxeGrid({
  ...
  } as VxeTableGridOptions<InsuranceCompanyApi.Company>,
});

function onCreate() {
  formModalApi.setData({}).open();
}
function onEdit(row: InsuranceCompanyApi.Company) {
  formModalApi.setData(row).open();
}
function onDelete(row: InsuranceCompanyApi.Company) {
  deleteInsuranceCompany(row.id!).then(() => {
