...
Modules (one per resource): `customers`, `vehicles`, `policies`, `renewals`, `users` (+`AuthService` for login), `logs` (read-only audit feed), `stats` (read-only `GET /api/v1/stats/dashboard`), `commercial-insurance-types`, `compulsory-insurance-types`, `insurance-companies`, `upload`, `ocr`. Entity chain: `customer` 1→N `vehicle` 1→N `policy` 1→N `renewal_record`. Reference tables auto-seed default rows inside `initSchema()` when empty.
...
- **Auth is NOT enforced** (deliberate). `POST /api/v1/users/login` validates SHA-256 and returns the user object — **no token/session/JWT, no guard**. All routes stay open (the frontend stores a user in localStorage under `driveease_user` with a fake token; enforcing JWT would break it). `LogService` defaults `operator` to `'管理员'` for write audit (real-caller identity not wired) — match this unless asked to change.
